...
EOF
  ...
  # Create page using Confluence REST API
  ...
  echo "Confluence page created: $title"
}
...
# Function to share page with team
share_page_with_team() {
  local page_id=$1
  local team=$2
  
  echo "Sharing page with team: $team..."
...
EOF
  ...
  echo "Page shared with team: $team"
}

# Function to notify team about Confluence page
notify_team_about_page() {
  local team=$1
  local lead=$2
  local page_title=$3
  local page_url=$4
  
  echo "Notifying $lead about Confluence page..."
...
I've created a Confluence live doc for Boomerang Tasks implementation. This document allows for real-time collaboration among all teams.
...
# Create main Boomerang Tasks page
...
<p>This is a <strong>live collaboration document</strong> - all teams should contribute their implementation details, questions, and feedback in real-time.</p>
...
# Create main page
MAIN_PAGE_RESPONSE=$(create_confluence_page "$MAIN_PAGE_TITLE" "$MAIN_PAGE_CONTENT" "$PARENT_PAGE_ID")
...
echo "Main page created with ID: $MAIN_PAGE_ID"
echo "Main page URL: $MAIN_PAGE_URL"

# Share page with all teams
for team in "${!TEAM_LEADS[@]}"; do
  share_page_with_team "$MAIN_PAGE_ID" "$team"
done

# Notify all teams
for team in "${!TEAM_LEADS[@]}"; do
  lead="${TEAM_LEADS[$team]}"
  notify_team_about_page "$team" "$lead" "$MAIN_PAGE_TITLE" "$MAIN_PAGE_URL"
done
