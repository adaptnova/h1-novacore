#!/bin/bash
# Script to install Google Chrome and Chrome Remote Desktop
# Created: 2025-03-29
...
print_section "Installing Google Chrome and Chrome Remote Desktop"

# Copy the script to the adapt server
print_info "Copying script to adapt server..."
cat > install_chrome_crdt_remote.sh << 'EOF'
#!/bin/bash
...
chmod +x install_chrome_crdt_remote.sh
scp -i /home/x/.ssh/ibm-admin_rsa install_chrome_crdt_remote.sh root@10.240.1.6:/root/
if [ $? -eq 0 ]; then
...
ssh -i /home/x/.ssh/ibm-admin_rsa root@10.240.1.6 "chmod +x /root/install_chrome_crdt_remote.sh && /root/install_chrome_crdt_remote.sh"
if [ $? -eq 0 ]; then
...
print_section "Summary"
print_success "Google Chrome and Chrome Remote Desktop have been installed on adapt server."
print_info "GNOME has been set as the default desktop environment."
print_info "To complete the setup, Chase needs to:"
print_info "1. Log in to the adapt server as user 'x'"
print_info "2. Open Google Chrome and log in to their Google account"
