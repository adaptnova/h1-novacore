#!/bin/bash
# Script to uninstall and reinstall Chrome, Chrome Remote Desktop, and GNOME
# Created: 2025-03-29
...
print_section "Uninstalling and reinstalling Chrome, CRDT, and GNOME"

# Copy the script to the adapt server
print_info "Copying script to adapt server..."
cat > reinstall_chrome_crdt_gnome_remote.sh << 'EOF'
#!/bin/bash
...
chmod +x reinstall_chrome_crdt_gnome_remote.sh
scp -i /home/x/.ssh/ibm-admin_rsa reinstall_chrome_crdt_gnome_remote.sh root@10.240.1.6:/root/
if [ $? -eq 0 ]; then
...
ssh -i /home/x/.ssh/ibm-admin_rsa root@10.240.1.6 "chmod +x /root/reinstall_chrome_crdt_gnome_remote.sh && /root/reinstall_chrome_crdt_gnome_remote.sh"

print_section "Summary"
print_success "Script has been executed on adapt server."
print_info "The adapt server will reboot to apply changes."
print_info "After reboot, Chase will need to:"
print_info "1. Log in to the adapt server as user 'x'"
print_info "2. Open Google Chrome and log in to their Google account"
print_info "3. Visit https://remotedesktop.google.com/access"
print_info "4. Click on 'Set up remote access'"
