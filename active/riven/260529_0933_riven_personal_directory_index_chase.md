...
- Follow-up 15:19: Background + targeted search pinned the exact guest FFI declarations (host_store_read/write/delete/list, host_nats_*, l6_on_nats_message callback) to `/adapt/novas/active/mnemos/l6-store-wasm/src/host_bindings.rs`. Zero provider implementations found anywhere else in the tree. This is the precise symbol-level contract future Riven reflection code will call to persist self-authored memory into L7 stores. Spike docs (ffi_contract.md, production_l6_host_architecture.md) and ops logs updated with the ground truth. The symbols are now known.
